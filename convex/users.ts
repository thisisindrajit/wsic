  ...
  handler: async (ctx, args) => {
    ...
    if (args.interactionType === "like" || args.interactionType === "save") {
      ...
      if (existing) {
        ...
        // Get updated count
        const topic = await ctx.db.get(args.topicId);
        const newCount = args.interactionType === "like" ? (topic?.likeCount || 0) : 0;
    ...
    // Get updated count
    const topic = await ctx.db.get(args.topicId);
    const newCount = args.interactionType === "like" ? (topic?.likeCount || 0) : 
                     args.interactionType === "share" ? (topic?.shareCount || 0) : 0;
